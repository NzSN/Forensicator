...
pub fn decode_memory_info_list(
    data: &[u8],
    prov: Provenance,
) -> Result<Vec<RawMemoryInfoEntry>, Anomaly> {
    if data.len() < 16 {
        return Ok(vec![]);
    }
    ...
    if data.len() < expected_len {
        return Err(Anomaly {
            provenance: prov,
            description: format!(
                "truncated MemoryInfoList: expected {expected_len}, got {}",
                data.len()
            ),
        });
    }
    ...
    for i in 0..count {
        ...
        entries.push(RawMemoryInfoEntry {
            va_start,
            size,
            protection,
            state,
